...
async function GetAllStudent(){
    const students = await eel.GetStudentsInfo()()
...
async function ModalEdit(lrn){
    const studentInfo = await eel.getSpecificStudent(lrn)()
    const modal = document.getElementById("addStudentModal")
    modal.classList.add('hidden')
    modal.classList.remove("flex")
    const profile = document.getElementById("profile-pic")
    studentInfo[0][4] === "Male" ? (profile.src = "./public/Male.png"):(profile.src = "./public/Female.png")
    modalFlex.classList.add('flex')
    modalFlex.classList.remove("hidden")
    const select = document.createElement("select")
    select.id = "edit-course-input"
    select.classList.add("bg-slate-900", "p-2", "text-center", "w-96")
...
function setEditInputValue(studentInfo){
    document.getElementById("edit-lrn-input").value = studentInfo[0][0]
    document.getElementById("edit-fname-input").value = studentInfo[0][1]
    document.getElementById("edit-lname-input").value = studentInfo[0][3]
    document.getElementById("edit-mname-input").value = studentInfo[0][2]
    document.getElementById("edit-age-input").value = studentInfo[0][5]
    document.getElementById("edit-gender-input").value = studentInfo[0][4];
    document.getElementById("edit-year-input").value = studentInfo[0][6];
    document.getElementById("edit-course-input").value = studentInfo[0][8]
...
function UpdateStudent(){
   ...
   if(fname.length === 0 || mname.length === 0 || lname.length === 0 || age.length === 0){
    ...
    const studInfo = [lrn,fname,mname,lname,gender,age,year,course,courseCode]
    Update(lrn,studInfo)
...
async function AddStudent(){
    const lrn = document.getElementById("add-lrn-input").value
    const isUnique = await eel.getSpecificStudent(lrn)()
    const fname = document.getElementById("add-fname-input").value
    const mname = document.getElementById("add-mname-input").value
    const lname = document.getElementById("add-lname-input").value
   ...
   if(lrn.length === 0 || fname.length === 0 || mname.length === 0 || lname.length === 0 || age.length === 0){
        swal("Nope!", "Don't leave any inputs empty", "error");
   }else if(lrn.length != 12){
        swal("Uh! OH!", "Lrn must be 12 digits ", "error");
   }else if(isUnique.length != 0){
    ...
    Add(lrn,fname,mname,lname,gender,age,year,course,courseCode)
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          ...
          document.getElementById(`${lrn}-tr`).remove()
        } else {
...
async function Add(lrn,fname,mname,lname,gender,age,year,course,coursecode){
    const info = [lrn,fname,mname,lname,gender,age,year,course,coursecode]
    console.log(info)
    await eel.AddStudent(info)
}
...
async function lrnSearchResults(lrn){
    const results  = await eel.lrnSearch(lrn)()
    displaySearchResults(results)
}

async function nameSearchResults(name){
    const results  = await eel.nameSearch(name)()
    displaySearchResults(results)
}
