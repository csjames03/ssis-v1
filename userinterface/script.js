...
async function getAllCourses(){
    ...
    for(let i=0; i<courses.length; i++){
        ...
        option.value = courses[i][1]
        option.innerText = courses[i][2]
...
async function getResults(searchValue){
    ...
    if(resultsname.length > 0){
        for(let i = 0; i < resultsname.length; i++){
            ...
            div.innerHTML =`
                <img src="${resultsname[i][5] === 'Male'? ('./public/Male.png'):('./public/Female.png')}" alt="profile" class="w-10 h-10 rounded-full cols-span-1">
                <div class="text-base font-semibold col-auto flex flex-row">${resultsname[i][1]}  ${resultsname[i][3]}</div>
                <p class="text-base font-semibold col-auto">${resultsname[i][6]}</p>
        ...
        for(let i = 0; i < resultslrn.length; i++){
            ...
            div.innerHTML =`
                <img src="${resultslrn[i][5] === 'Male'? ('./public/Male.png'):('./public/Female.png')}" alt="profile" class="w-10 h-10 rounded-full cols-span-1">
                <div class="text-base font-semibold col-auto flex flex-row">${resultslrn[i][1]}  ${resultslrn[i][3]}</div>
                <p class="text-base font-semibold col-auto">${resultslrn[i][6]}</p>
...
function PutValues(student){
    ...
    dob.value = student[4]
    gender.value = student[5]
    year.value = student[6]
    code.value = student[7]
...
//Update Student 
async function Update(newInfo){
    const update = await eel.UpdateStudent(newInfo[0], newInfo)
    if(update){
        Swal.fire({
            position: 'top-end',
            icon: 'success',
          ...
          Hide("editModal")
          const profile = document.getElementById(`${newInfo[0]}-prof`)
          const Full = document.getElementById(`${newInfo[0]}-Full`)
          const gender = document.getElementById(`${newInfo[0]}-gender`)
          const age = document.getElementById(`${newInfo[0]}-age`)
          const agenumber = calculateAge(newInfo[4])
          const year  = document.getElementById(`${newInfo[0]}-year`)
          const course = document.getElementById(`${newInfo[0]}-course`)
          profile.src = newInfo[5] === 'Male'?('./public/Male.png'):('./public/Female.png')
          Full.innerHTML = `${newInfo[1]} ${newInfo[2]} ${newInfo[3]}`
          gender.innerHTML = newInfo[5]
...
document.getElementById('editForm').addEventListener('submit', (e)=>{
    ...
    const info = [lrn,fname,mname,lname,dob,gender,year,code]
...
async function Add(student){
    ...
    if(add){
        ...
        Hide("addModal")
        const lrn = document.getElementById('add-lrn')
...
document.getElementById('addForm').addEventListener('submit', async (e)=>{
    e.preventDefault()
    const lrn = document.getElementById('add-lrn').value
    const isUnique = await eel.IsUnique(lrn)()
    ...
    if(fname.length === 0 || mname.length === 0 || lname.length === 0 || dob.length === 0 ||gender.length === 0 || year.length === 0 || code.length === 0){
        ...
        Swal.fire({
            title: 'Error!',
            text: "HMMMM.... It looks like this LRN is taken",
            icon: 'error',
            confirmButtonText: 'Retry'
          })
        ...
        Swal.fire({
            title: 'Error!',
            text: "Uhh OH! LRN must be 12 digit numbers",
            icon: 'error',
            confirmButtonText: 'Retry'
          })
    }else{
...
function DisplayStudent(student){
    ...
    const age = calculateAge(student[4])
    const div = document.createElement('div')
    ...
    <div class="col-span-1">
        <img id="${student[0]}-prof" class="h-10" src="${student[5] === 'Male'? ('./public/Male.png'):('./public/Female.png')}" />
    </div>
    <div id="${student[0]}-Full" class="col-span-2 font-semibold truncate">${student[1]} ${student[2]} ${student[3]}</div>
    <div id="${student[0]}-gender" class="col-span-1">${student[5]}</div>
    <div id="${student[0]}-age" class="col-span-1">${age}</div>
    <div id="${student[0]}-year" class="col-span-1">${student[6]}</div>
    <div id="${student[0]}-course" class="col-span-1">${student[7]}</div>
    `
    div.addEventListener('click', function(){
        Show('editModal')
        if(prev !== undefined){
